<?php
	
	// Business tier class that manages customer accounts functionality
	class Customer
	{			
		// Checks if a customerID exists in session
		public static function IsAuthenticated()
		{
			if (!(isset ($_SESSION['emart_customer_id'])))
				return 0;
			else
				return 1;
		}
		
		// Returns customerID and password for customer with email $email
		public static function GetLoginInfo($email)
		{
			// Build the SQL query
			$sql = 'CALL customer_get_login_info(:email)';
			
			// Build the parameters array
			$params = array (':email' => $email);
			
			// Execute the query and return the results
			return DatabaseHandler::GetRow($sql, $params);
		}
		
		public static function IsValid($email, $password)
		{
			$customer = self::GetLoginInfo($email);
			
			if (empty ($customer['customerID']))
				return 2;
			
			$customer_id = $customer['customerID'];
			$hashed_password = $customer['password'];
			
			if (PasswordHasher::Hash($password) != $hashed_password)
				return 1;
			else
			{
				$_SESSION['emart_customer_id'] = $customer_id;
				return 0;
			}
		}
		
		public static function Logout()
		{
			unset($_SESSION['emart_customer_id']);
		}
		
		public static function GetCurrentCustomerId()
		{
			if (self::IsAuthenticated())
				return $_SESSION['emart_customer_id'];
			else
				return 0;
		}
		
		/* Adds a new customer account, log them in if $addAndLogin is true
		and returns customerID */
		public static function Add($firstname, $surname, $email, $password, $addressLineOne,
									$town, $city, $county, $postcode, $telephone, $addAndLogin = true)
		{
			$hashed_password = PasswordHasher::Hash($password);
			
			// Build the SQL query
			$sql = 'CALL customer_add(:firstname, :surname, :email, :password, :unencPassword, 
									:addressLineOne, :town, :city, :county, 
									:postcode, :telephone)';
			
			// Build the parameters array
			$params = array (':firstname' => $firstname, ':surname' => $surname, ':email' => $email,
								':password' => $hashed_password, ':unencPassword' => $password,
								':addressLineOne' => $addressLineOne, ':town' => $town, 
								':city' => $city, ':county' => $county, 
								':postcode' => $postcode, ':telephone' => $telephone);
			
			// Execute the query and get the customer_id
			$customer_id = DatabaseHandler::GetOne($sql, $params);
			
			if ($addAndLogin)
				$_SESSION['emart_customer_id'] = $customer_id;

			return $customer_id;
		}		
		
		public static function GetUnencryptedPassword($customer_email)
		{
			// Build the SQL Query
			$sql = 'CALL customer_get_unencPassword(:email)';
			
			// Build the parameters array
			$params = array(':email' => $customer_email);
			
			// Execute the query and return the results
			return DatabaseHandler::GetOne($sql, $params);
		
		}
		
		
		// Get the details for a customer
		public static function GetCustomerDetails($customerId)
		{
			// Build the SQL query
			$sql = 'CALL customer_get_customer(:customer_id)';
			
			// Build the parameters array
			$params = array (':customer_id' => $customerId);
			
			// Execute the query and return the results
			return DatabaseHandler::GetRow($sql, $params);
		}
		
		
		public static function Get($customerId = null)
		{
			if (is_null($customerId))
				$customerId = self::GetCurrentCustomerId();
			
			// Build the SQL query
			$sql = 'CALL customer_get_customer(:customer_id)';
			
			// Build the parameters array
			$params = array (':customer_id' => $customerId);
			
			// Execute the query and return the results
			return DatabaseHandler::GetRow($sql, $params);
		}
		
		public static function UpdateAccountDetails($firstname, $surname, $email, $password,
		$telephone, $addressLineOne, $town, $city, $county, $postcode, $customerId = null)
		{
			if (is_null($customerId))
				$customerId = self::GetCurrentCustomerId();
			
			$hashed_password = PasswordHasher::Hash($password);
			
			// Build the SQL query
			$sql = 'CALL customer_update_account(:customer_id, :firstname, :surname, :email,
							:password, :unencPassword, :telephone, :addressLineOne, :town, :city, :county, :postcode)';
			
			// Build the parameters array
			$params = array (':customer_id' => $customerId, ':firstname' => $firstname,
							':surname' => $surname, ':email' => $email, 
							':password' => $hashed_password, 
							':unencPassword' => $password, ':telephone' => $telephone,
							':addressLineOne' => $addressLineOne, ':town' => $town, 
							':city' => $city, ':county' => $county, ':postcode' => $postcode);
			
			// Execute the query
			DatabaseHandler::Execute($sql, $params);
		}
		
		/* Adds a new customer account */
		public static function AddCustomer($firstname, $surname, $email, $password, $unencPassword, $addressLineOne,
									$town, $city, $county, $postcode, $telephone)
		{						
			// Build the SQL query
			$sql = 'CALL customer_add(:firstname, :surname, :email, :password, :unencPassword, 
									:addressLineOne, :town, :city, :county, 
									:postcode, :telephone)';
			
			// Build the parameters array
			$params = array (':firstname' => $firstname, ':surname' => $surname, ':email' => $email,
								':password' => $password, ':unencPassword' => $unencPassword,
								':addressLineOne' => $addressLineOne, 
								':town' => $town, ':city' => $city, ':county' => $county, 
								':postcode' => $postcode, ':telephone' => $telephone);
			
			// Execute the query and get the customer_id
			$customer_id = DatabaseHandler::GetOne($sql, $params);
		}
		
		public static function UpdateCustomer($customerId, $firstname, $surname, $email, $password, $unencPassword,
		$telephone, $addressLineOne, $town, $city, $county, $postcode)
		{
			
			// Build the SQL query
			$sql = 'CALL customer_update_account(:customer_id, :firstname, :surname, :email,
							:password, :unencPassword, :telephone, :addressLineOne, 
							:town, :city, :county, :postcode)';
			
			// Build the parameters array
			$params = array (':customer_id' => $customerId, ':firstname' => $firstname,
							':surname' => $surname, ':email' => $email, 
							':password' => $password, ':unencPassword' => $unencPassword,
							':telephone' => $telephone,
							':addressLineOne' => $addressLineOne, ':town' => $town, 
							':city' => $city, ':county' => $county, ':postcode' => $postcode);
			
			// Execute the query
			DatabaseHandler::Execute($sql, $params);
		}
		
		
		public static function DecryptCreditCard($encryptedCreditCard)
		{
			$secure_card = new SecureCard();
			$secure_card->LoadEncryptedDataAndDecrypt($encryptedCreditCard);
			$credit_card = array();
			$credit_card['card_holder'] = $secure_card->CardHolder;
			$credit_card['card_number'] = $secure_card->CardNumber;
			$credit_card['issue_date'] = $secure_card->IssueDate;
			$credit_card['expiry_date'] = $secure_card->ExpiryDate;
			$credit_card['issue_number'] = $secure_card->IssueNumber;
			$credit_card['card_type'] = $secure_card->CardType;
			$credit_card['card_number_x'] = $secure_card->CardNumberX;
			
			return $credit_card;
		}
		
		public static function GetPlainCreditCard()
		{
			$customer_data = self::Get();
			if (!(empty ($customer_data['creditCard'])))
				return self::DecryptCreditCard($customer_data['creditCard']);
			else
				return array('card_holder' => '', 'card_number' => '',
							'issue_date' => '', 'expiry_date' => '',
							'issue_number' => '', 'card_type' => '',
							'card_number_x' => '');
		}
		
		public static function UpdateCreditCardDetails($plainCreditCard,
		$customerId = null)
		{
			if (is_null($customerId))
				$customerId = self::GetCurrentCustomerId();
			
			$secure_card = new SecureCard();
			$secure_card->LoadPlainDataAndEncrypt($plainCreditCard['card_holder'],
			$plainCreditCard['card_number'], $plainCreditCard['issue_date'],
			$plainCreditCard['expiry_date'], $plainCreditCard['issue_number'],
			$plainCreditCard['card_type']);
			$encrypted_card = $secure_card->EncryptedData;
		
			// Build the SQL query
			$sql = 'CALL customer_update_credit_card(:customer_id, :creditCard)';
		
			// Build the parameters array
			$params = array (':customer_id' => $customerId,
							':creditCard' => $encrypted_card);
			
			// Execute the query
			DatabaseHandler::Execute($sql, $params);
		}
		
		// Remove customer from the database
		public static function RemoveCustomer($customerId)
		{
			//Build the SQL Query
			$sql = 'CALL customer_delete_customer(:customer_id)';
			
			// Build the parameters array
			$params = array (':customer_id' => $customerId);
			
			// Execute the query
			DatabaseHandler::Execute($sql, $params);
		}
		
		// Gets all customers names with their associated id
		public static function GetCustomersList()
		{
			// Build the SQL query
			$sql = 'CALL customer_get_customers_list()';
			
			// Execute the query and return the results
			return DatabaseHandler::GetAll($sql);
		}
	}
?>