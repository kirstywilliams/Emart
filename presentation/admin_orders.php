		...
		public function init()
		{
			...
			if (isset ($_GET['submitOrdersByStatus']))
			{
				...
				if ($this->mSelectedStatus == 0)
					$this->mOrders = Orders::GetOrdersByStatus('Received');
				elseif($this->mSelectedStatus == 1)
					$this->mOrders = Orders::GetOrdersByStatus('Checking Funds');
				elseif($this->mSelectedStatus == 2)
					$this->mOrders = Orders::GetOrdersByStatus('Checking Stock');
				elseif($this->mSelectedStatus == 3)
					$this->mOrders = Orders::GetOrdersByStatus('Stock Ok');
				elseif($this->mSelectedStatus == 4)
					$this->mOrders = Orders::GetOrdersByStatus('Taking Payment');
				elseif($this->mSelectedStatus == 5)
					$this->mOrders = Orders::GetOrdersByStatus('Awaiting Despatch');
				elseif($this->mSelectedStatus == 6)
					$this->mOrders = Orders::GetOrdersByStatus('Despatched');
				elseif($this->mSelectedStatus == 7)
					$this->mOrders = Orders::GetOrdersByStatus('Complete');
				elseif($this->mSelectedStatus == 8)
					$this->mOrders = Orders::GetOrdersByStatus('Cancelled');
				else
					$this->mErrorMessage = 'Unrecognised searching criteria!';
			}
			...
			for ($i = 0; $i < count($this->mOrders); $i++)
			{
				...
				Link::ToOrderDetailsAdmin($this->mOrders[$i]['orderID']);
			}
		}
	}
?>